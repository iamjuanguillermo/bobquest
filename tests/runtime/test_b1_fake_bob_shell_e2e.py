...
def init_local_git_repo(path: Path) -> None:
    ...
    subprocess.run(["git", "add", "README.md", "package.json"], cwd=path, check=True)
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="bobquest-b1-") as tmpdir:
        tmp = Path(tmpdir)
        ...
        try:
            ...
            status, capabilities = http_json("GET", f"{base_url}/api/capabilities")
            assert status == 200, capabilities
            assert capabilities["bob_shell_runtime"]["available"] is True
            assert capabilities["public_demo_mode"] is True

            status, created = http_json("POST", f"{base_url}/api/runs", {"repo_id": "acme/bobquest-demo"})
            assert status == 202, created
            run_id = created["run_id"]
            assert created["state"] == "creating_workspace"
            ...
            assert state["analysis_original"]["repo"]["name"] == "bobquest-demo"
            assert state["analysis_original"]["recommended_first_flow_id"] == "repo-health-check"
            assert state["analysis_original"]["flows"][0]["starter_missions"][0]["id"] == "confirm-health-check-command"
            assert state["error"] is None

            workspace_root = tmp / "runtime" / "workspaces" / run_id / "repo"
            assert (workspace_root / "README.md").exists(), "expected shallow-cloned repository workspace"
        finally:
