...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="bobquest-b3-") as tmpdir:
        tmp = Path(tmpdir)
        ...
        try:
            ...
            assert workspace["workspace_id"] == run_id, workspace
            assert workspace["status"] == "cloned", workspace
            assert workspace["clone_started_at"], workspace
            assert workspace["clone_finished_at"], workspace
            assert workspace["cleanup_started_at"] is None, workspace
            assert workspace["cleanup_finished_at"] is None, workspace
            assert Path(workspace["repo_dir"]).joinpath("README.md").exists(), workspace

            run_state_file = runtime_dir / run_id / "run_state.json"
            persisted = json.loads(run_state_file.read_text(encoding="utf-8"))
            assert persisted["workspace"]["repo_dir"] == workspace["repo_dir"], persisted
        finally:
