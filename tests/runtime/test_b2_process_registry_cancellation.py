...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="bobquest-b2-") as tmpdir:
        tmp = Path(tmpdir)
        ...
        env.update(
            {
                ...
                "BOBQUEST_FAKE_BOB_SLEEP_MS": "10000",
        ...
        try:
            ...
            while time.time() < deadline:
                ...
                if state["state"] == "running_bob_analysis":
                    break
                assert state["state"] not in {"failed", "ready"}, state
                time.sleep(0.25)
            assert state["state"] == "running_bob_analysis", state

            status, cancelled = http_json("POST", f"{base_url}/api/runs/{run_id}/cancel")
            assert status == 200, cancelled
            assert cancelled["state"] == "cancelled", cancelled
            assert cancelled["error"] is None

            # Give the killed fake Bob Shell enough time to close. The run must not be overwritten as failed/ready.
            time.sleep(3)
            status, final_state = http_json("GET", f"{base_url}/api/runs/{run_id}")
            assert status == 200, final_state
            assert final_state["state"] == "cancelled", final_state
            assert final_state["analysis_original"] is None, final_state
        finally:
